...
SELECT m.name, m.brand_name, b.headquarters FROM models AS m JOIN brands AS b ON m.brand_name = b.name WHERE m.name = 'Mustang';
...
SELECT m.name, m.brand_name, b.founded FROM models AS m LEFT JOIN brands AS b ON m.brand_name = b.name WHERE m.year = 1960;
...
SELECT m.brand_name, b.founded, m.name FROM brands AS b JOIN models AS m ON b.name = m.brand_name WHERE b.discontinued IS NULL;SELECT m.name, m.brand_name, b.founded FROM models AS m JOIN brands AS b ON b.name = m.brand_name;
...
SELECT brand_name FROM models AS m JOIN brands AS b ON m.brand_name = b.name GROUP BY brand_name HAVING COUNT(*) > 5;
...
SELECT m.name, m.year FROM models AS m JOIN brands AS b ON m.brand_name = b.name WHERE m.year IN (SELECT founded FROM brands);